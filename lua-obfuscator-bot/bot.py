...
config_token = ""                       # Your discord bottoken
...
config_noluafile = "Please attach a `.lua` file!"
config_obf_success = "**Here is your obfuscated code, {message.author.mention}! It took {obfuscation_time} seconds to obfuscate!**"

config_obf_cmd = "!obfuscate"
...
async def on_message(message):
    ...
    if message.content.startswith(config_obf_cmd):
        ...
        if not attachment.filename.lower().endswith('.lua'):
            ...
            embed = discord.Embed(title=config_failtitle, description=config_noluafile, color=discord.Color.red())
            embed.set_footer(text=config_footer, icon_url=config_icon_url)
            embed.set_thumbnail(url=config_thumbnail_url)
            await message.channel.send(embed=embed)
            return
